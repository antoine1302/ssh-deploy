...
rsync -avzh --exclude '.git' -e "ssh -i /github/home/.ssh/id_rsa -p $INPUT_PORT" ./ "$INPUT_USERNAME@$INPUT_HOST:$path_on_remote/"
ssh -i /github/home/.ssh/id_rsa "$INPUT_USERNAME@$INPUT_HOST" -p "$INPUT_PORT" ln -s "$path_on_remote" "/home/$INPUT_USERNAME/projects/$INPUT_REPOSITORY"
